...
#>
using namespace 'ghcr.io/startautomating/wherefor'
...
if ($args) {
    ...
    {        
        ...
        $whereForFiles = $mountedFolders | ? Name -match '\.WhereFor\.ps1'
        $whereforJobs = foreach ($whereForFile in $whereForFiles) {
            Start-ThreadJob -Name $whereForFile.Name -ScriptBlock {param($whereForFile) . $using:whereForFile.FullName } -ArgumentList $whereForFile            
        }
        $whereforJobs | Wait-Job | Receive-Job
    }
    #endregion Custom
}
